...
def deterministic_query_sample(
    ...
    query_ids = sorted(
        int(query_id)
        for query_id in proposals["query_id"]
        .drop_duplicates()
        .tolist()
    )

    if len(query_ids) < QUERIES_PER_SEED:
        raise RuntimeError(
            "Fewer than three placement-feasible proposal queries."
        )
    ...
    selected_ids = [
        query_ids[int(position)]
        for position in positions
    ]

    if len(set(selected_ids)) != QUERIES_PER_SEED:
        raise RuntimeError(
            "Deterministic pilot query selection duplicated an index."
        )

    selected = proposals.loc[
        proposals["query_id"].isin(selected_ids)
    ].copy()
    ...
    if len(selected) != QUERIES_PER_SEED:
        raise RuntimeError(
            "Pilot query selection did not return exactly three proposals."
        )
...
if tuple(
    int(seed)
    for seed in charter["v5b_pilot_scope"][
        "allowed_model_training_pilot_seeds"
    ]
) != PILOT_SEEDS:
    raise RuntimeError(
        "V5-B pilot registry mismatch."
    )
...
if proposal_manifest["teacher_or_oracle_columns_used"].any():
    raise RuntimeError(
        "V5-B proposal selection used forbidden labels."
    )
...
action_effects_path = (
    output_root
    / "v5b_dynamic_action_effect_labels.csv"
)

proposal_manifest_path = (
    output_root
    / "v5b_runtime_safe_proposal_manifest.csv"
)

daily_path = (
    output_root
    / "v5b_dynamic_action_effect_daily.csv"
)

seed_runs_path = (
    output_root
    / "v5b_seed_execution_manifest.json"
)
...
summary_by_action_path = (
    output_root
    / "v5b_action_effect_summary_by_placement.csv"
)
...
decision_path = (
    output_root
    / "v5b_direct_action_effect_decision.json"
)

write_json(decision_path, decision)

print("===== V5-B DIRECT ACTION-EFFECT PILOT =====")
print(
    json.dumps(
        {
            "decision": decision,
