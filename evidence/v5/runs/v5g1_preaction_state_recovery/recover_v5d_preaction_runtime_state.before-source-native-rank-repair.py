...
def query_geometry(
    ...
    work = work.sort_values(
        ["seed", "query_id", "base_score", "product_id"],
        ascending=[True, True, False, True],
        kind="mergesort",
    ).reset_index(drop=True)

    work["canonical_rank"] = (
        work.groupby(["seed", "query_id"]).cumcount() + 1
    )
    ...
    boundaries = (
        work.loc[
            work["canonical_rank"].isin(TARGET_RANKS),
            [
                "seed",
                "query_id",
                "canonical_rank",
    ...
    geometry = geometry.merge(
        above,
        on=["seed", "query_id", "canonical_rank"],
        how="left",
        validate="one_to_one",
    ).merge(
        below,
